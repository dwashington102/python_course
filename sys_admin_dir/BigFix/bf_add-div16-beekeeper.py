...
file, performs an ldapsearch using the IntranetID in order to pull
the user's division number.

The division number is then appended to the
/var/opt/beekeeper/beekeeper.ini file in this format:
div=16


Exit Codes:
    1 -
    2 - Unable to create /tmp/bf_linustatibm directory
...
def check_beekeeper():
    '''
    May be necessary to add a try-except to confirm
    /var/opt/beekeeper/beekeeper.ini exists
    '''


def check_bf_tmpdir():
    tmp_path = '/tmp/bf_linuxatibm'
    chkdir = os.path.exists(tmp_path)
    log.info('DEBUG >>> chkdir = {}'.format(chkdir))
    if not chkdir:
        try:
            os.makedirs(tmp_path)
            log.info('Created {}'.format(tmp_path))
        except IOError:
            log.info('Unable to create...exit(2)'.format(tmp_path))
            sys.exit(2)
        


def get_ldapInfo():
    ''' May be necessary to add try-except to confirm LDAP
    connection before performing steps
    ...
    resultList = l.search_s(ldapBase,ldap.SCOPE_SUBTREE,ldapSearch,['ibm-allgroups'])
...
def update_beekeeper(beekeeperEntry):
    '''
    - "div=16" text is written to /tmp/beekeeper.ini
    - The original /var/opt/beekeeper/beekeeper.ini file
    is moved to /tmp/bf_linuxatibm/beekeeper.ini.
    - /tmp/beekeeper.ini is moved to /var/opt/beekeeper/beekeper.ini
    '''
