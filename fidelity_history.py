...
To sum the values of the lists - we have to take the 1st elememt of the list
(element 0) convert the element to a float before adding the element to the
total dividend NOTE:  If any 0 element contains a comma ",", Python will read
the element as a string which cannot be converted to a float.  In order to
avoid this issue we must use the replace() to remove unwanted commas

Steps to create the csv file
> Login to Fidelity --> Download transactions for selected time frame
--> save to csv file
...
def get_filename():    # Prompt the user for the path to the csv file
    ...
    try:
        ...
        print(f"File not found: {u_history}")
        print()
        exit(1)
...
def compute_totals(u_history):
    ...
    with open(u_history, 'r') as afile:
        ...
        for r in row:
            # Converts r to string in order to use RE when searching for transaction type
            rowstring = str(r)
            if re.search(r"Dividend", rowstring, re.IGNORECASE):
                # Isolate DIVIDEND rows and total
                my_div = r[3]
                my_div = float(my_div.replace(',', ''))
                tot_div = tot_div + my_div
            elif re.search('Contribution', rowstring, re.IGNORECASE):
                # Isolate CONTRIBUTION rows and total
                my_contr = r[3]
                my_contr = float(my_contr.replace(',', ''))
                tot_contr = tot_contr + my_contr
            elif re.search('ADMINISTRATIVE FEES', rowstring, re.IGNORECASE):
                # Isolate ADMINISTRATIVE FEES rows and total
                my_fees = r[3]
                my_fees = float(my_fees.replace(',', ''))
                tot_fees = tot_fees + my_fees
            elif re.search('REVENUE CREDIT', rowstring, re.IGNORECASE):
                # Isolate REVENUE CREDIT rows and total
                my_credit = r[3]
                my_credit = float(my_credit.replace(',', ''))
                tot_credit = tot_credit + my_credit
            elif re.search('Loan Repayments', rowstring, re.IGNORECASE):
                # Isolate Loan Repayments rows and total
                my_loan_repay = r[3]
                my_loan_repay = float(my_loan_repay.replace(',', ''))
                tot_loan_repay = tot_loan_repay + my_loan_repay
            elif re.search('Loan Origination', rowstring, re.IGNORECASE):
                # Isolate Loan rows and total
                my_loan_ori = r[3]
                my_loan_ori = float(my_loan_ori.replace(',', ''))
                tot_loan_ori = tot_loan_ori + my_loan_ori
            elif re.search('Loan Setup Fee', rowstring, re.IGNORECASE):
                # Isolate Loan rows and total
                my_loan_fee = r[3]
                my_loan_fee = float(my_loan_fee.replace(',', ''))
                tot_loan_fee = tot_loan_fee + my_loan_fee
            elif re.search('Loan', rowstring, re.IGNORECASE):
                # Isolate Loan Repayments rows and total
                my_loan = r[3]
                my_loan = float(my_loan.replace(',', ''))
                tot_loan = tot_loan + my_loan
            elif re.search('Change in Market Value', rowstring, re.IGNORECASE):
                # Isolate Change in Market Value rows and total
                my_cmv = r[3]
                my_cmv = float(my_cmv.replace(',', ''))
                tot_cmv = tot_cmv + my_cmv
            elif re.search('Exchange out', rowstring, re.IGNORECASE):
                # Isolate Exchange out rows and total
                my_xout = r[3]
                my_xout = float(my_xout.replace(',', ''))
                tot_xout = tot_xout + my_xout
            elif re.search('Exchange In', rowstring, re.IGNORECASE):
                # Isolate Exchange In rows and total
                my_xin = r[3]
